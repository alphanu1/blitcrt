      ...
      unsigned height, size_t pitch)
{
   ...
   /* Mirror the finished frame to an attached FPGA/CRTPi CRT1 device.
    * No-op unless one was detected in video_crt_switch's init. Skip
    * hardware-buffer sentinels and duped (NULL) frames. */
   if (     data
         && data != RETRO_HW_FRAME_BUFFER_VALID)
      crt_send_frame(data, width, height, (unsigned)pitch);
